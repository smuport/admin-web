...
export class IconSelComponent implements OnInit, AfterViewInit {
  ...
  constructor() {
    ...
    this.iconUrl = '../../../../assets/icons/'; // 在这里初始化 iconUrl
  }
  ...
  selIconFn(item: IconItem): void {
    ...
    [
      this.sourceIconsArray,
      this.iconsStrShowArray,
  ...
  getData(event: number = this.pageObj.pageNum): void {
    ...
    this.iconsStrShowArray = [
      ...this.iconsStrAllArray.slice(
        (this.pageObj.pageNum - 1) * this.pageObj.pageSize,
        this.pageObj.pageNum * this.pageObj.pageSize
      ),
    ];
  ...
  ngAfterViewInit(): void {
    this.searchText$
      ...
      .subscribe((res) => {
        this.iconsStrAllArray = this.sourceIconsArray.filter((item) =>
          item.icon.includes(res)
        );
        this.getData();
        this.cdr.markForCheck();
      });
