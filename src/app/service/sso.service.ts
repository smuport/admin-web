...
export class SSOService implements OnDestroy {
  ...
  private handlePostMessage(event: MessageEvent): void {
    ...
    switch (event.data.type) {
      ...
      case "TOKEN_RECEIVED":
        ...
        break;
      case "LOGOUT":
        console.log("子系统确认登出:", event.origin);
        this.handleLogout();
        break;
    }
  }
  ...
  private sendTokenToChild(targetWindow: Window, targetOrigin: string): void {
    ...
    const refreshTokenExpires = window.localStorage.getItem("refreshTokenExpires");
    if (token) {
      targetWindow.postMessage(
        {
          type: "TOKEN_RESPONSE",
          token: `${TokenPre}${token}`,
          accessTokenExpires: accessTokenExpires ? parseInt(accessTokenExpires) : null,
          refreshTokenExpires: refreshTokenExpires ? parseInt(refreshTokenExpires) : null,
